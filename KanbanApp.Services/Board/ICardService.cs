...
{
    Task<Card> CreateCardAsync(string columnId, string title, string? description, string? color, string ownerId);
    Task<IEnumerable<Card>> GetColumnCardsAsync(string columnId, string ownerId);
    Task<Card> UpdateCardAsync(string cardId, string title, string? description, string? color, string ownerId);
